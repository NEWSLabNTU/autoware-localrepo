...
def generate_debian_files(version: str, output_dir: Path, suffix: str = "",
                          install_dir: str = "/opt/autoware/theme") -> None:
    """Generate debian packaging files."""
    pkg = f"autoware-theme{suffix}"
    ...
    changelog = f"""{pkg} ({version}-1) jammy; urgency=medium
...
\tinstall -d $(CURDIR)/debian/{pkg}{install_dir}
\tinstall -m 644 downloads/autoware.qss $(CURDIR)/debian/{pkg}{install_dir}/
\tinstall -m 644 downloads/qt5ct.conf $(CURDIR)/debian/{pkg}{install_dir}/
\tcp -r downloads/autoware-rviz-icons $(CURDIR)/debian/{pkg}{install_dir}/
\tinstall -d $(CURDIR)/debian/{pkg}/usr/bin
\tinstall -m 755 apply-autoware-theme $(CURDIR)/debian/{pkg}/usr/bin/
...
"""
    ...
    print(f"Generated debian files for {pkg} {version}")
...
def main():
    ...
    parser.add_argument(
        ...
        help="Package name suffix so multiple Autoware versions can coexist "
             "(e.g. -1-9-0 gives autoware-theme-1-9-0)"
    )
    parser.add_argument(
        "--install-dir",
        default="/opt/autoware/theme",
        help="Install prefix for theme files (default: /opt/autoware/theme). "
             "Use /opt/autoware/<version>/theme for versioned installs."
    )

    args = parser.parse_args()
    output_dir = Path(args.output)

    generate_debian_files(args.version, output_dir, args.suffix, args.install_dir)
