# Autoware 1.9.0 ARM64 Build

This directory contains the colcon2deb configuration for building Autoware 1.9.0 ROS packages on arm64 (native build on an arm64 host; the ghcr autoware-base:cuda-latest manifest carries an arm64 image).

## Configuration

### Package Naming

Packages are built with `-1-9-0` suffix to allow multiple Autoware versions to coexist:
- Example: `ros-humble-autoware-core-1-9-0` instead of `ros-humble-autoware-core`
...
Packages install to `/opt/autoware/1.9.0` instead of the default `/opt/ros/humble`.
...
# Harvest the generated debian/ dirs back into debian-overrides/
for d in build/build/*/; do
    pkg=$(basename "$d")
    [ -d "$d/debian" ] && mkdir -p "debian-overrides/$pkg" && cp -r "$d/debian" "debian-overrides/$pkg/"
done
