...
- `FindCUDA.cmake` - CMake wrapper bridging legacy CUDA API with modern CUDAToolkit
...
### FindCUDA.cmake Wrapper

The `FindCUDA.cmake` wrapper in `*/jp*/FindCUDA.cmake` bridges CMake's legacy CUDA API with modern CUDAToolkit. It solves multiple issues:

1. **CUDA::cudart target** - Legacy `find_package(CUDA)` doesn't create `CUDA::*` targets
2. **find_cuda_helper_libs macro** - Required by ROS `cudnn_cmake_module`
3. **cuda_add_library/cuda_add_executable** - Macros for CUDA compilation with proper flag handling
4. **Explicit CUDA architectures** - Avoids `native` detection that fails in QEMU
5. **C++ flag filtering** - Prevents GCC flags like `-Werror` from being passed to nvcc

The wrapper provides:
- Legacy variables: `CUDA_FOUND`, `CUDA_VERSION`, `CUDA_INCLUDE_DIRS`, `CUDA_LIBRARIES`
- Modern targets: `CUDA::cudart` and other `CUDA::*` targets via CUDAToolkit
- Macros: `cuda_add_library()`, `cuda_add_executable()`, `find_cuda_helper_libs()`
