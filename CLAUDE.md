...
Scripts installed to `/opt/autoware/1.5.0/`:

| Script                  | Purpose                                              |
|-------------------------|------------------------------------------------------|
| setup.$shell            | Pure ament workspace env (sources ROS + Autoware local_setup). Silent, no subprocesses. Safe for CI/scripts. |
| autoware-env.$shell     | Autoware runtime config (RMW_IMPLEMENTATION, CYCLONEDDS_URI, QT_QPA_PLATFORMTHEME). Requires setup.$shell first. |
| local_setup.$shell      | Standard ament local_setup (generated by colcon2deb) |
| _local_setup_util.py    | Standard ament utility (generated by colcon2deb)     |

**setup.sh AMENT_CURRENT_PREFIX fix**: ROS's `setup.sh` leaks `AMENT_CURRENT_PREFIX=/opt/ros/humble` when `AMENT_SHELL=sh` (sets it in its local_setup loop but never unsets it). Our `setup.sh` explicitly unsets it before sourcing Autoware's `local_setup.sh`. The bash/zsh variants are unaffected because `local_setup.bash`/`local_setup.zsh` determine their own path via `${BASH_SOURCE[0]}` and `${(%):-%N}`.

### Helper Scripts (in autoware-localrepo)

Scripts installed to `/usr/share/autoware/1.5.0/`:
...
- Applies sysctl settings from `/etc/sysctl.d/10-cyclone-max-1-5-0.conf`
- Enables multicast on loopback via `multicast-lo-1-5-0.service` or direct `ip link` (for Docker)
...
- `packages/autoware-ros-packages/debian/control` - Auto-generated by `genpkg.sh` from `build/dist/`
...
- `debian/postrm` - Post-removal script (for autoware-localrepo: cleans up versioned APT sources)

### Versioned System Paths

All files installed outside `/opt/autoware/1.5.0/` use version suffixes to allow multiple Autoware versions to coexist without dpkg file conflicts:

| File | Installed Path |
|------|---------------|
| Helper scripts | `/usr/share/autoware/1.5.0/` |
| APT source list | `/etc/apt/sources.list.d/autoware-localrepo-1-5-0.list` |
| APT preferences | `/etc/apt/preferences.d/autoware-localrepo-1-5-0.pref` |
| Sysctl config | `/etc/sysctl.d/10-cyclone-max-1-5-0.conf` |
| Systemd service | `/etc/systemd/system/multicast-lo-1-5-0.service` |
...
- Scans `build/dist/` directory for .deb files
