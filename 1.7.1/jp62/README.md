# Autoware 1.7.1 JetPack 6.2 (ARM64) Build

This directory contains the colcon2deb configuration for building Autoware 1.7.1 ROS packages on NVIDIA Jetson with JetPack 6.2.

## Configuration

### Package Naming

Packages are built with `-1-7-1` suffix to allow multiple Autoware versions to coexist:
- Example: `ros-humble-autoware-core-1-7-1` instead of `ros-humble-autoware-core`

This is configured in `config.yaml`:
```yaml
build:
  package_suffix: "1-7-1"
```

### Install Prefix

Packages install to `/opt/autoware/1.7.1` instead of the default `/opt/ros/humble`.
...
|---------|----------|--------|
| `autoware_mission_planner_universe` | `ros-humble-autoware-mission-planner-1-7-1` | Supersedes autoware_mission_planner |
| `autoware_overlay_rviz_plugin` | `ros-humble-autoware-mission-details-overlay-rviz-plugin-1-7-1` | Absorbed mission_details_overlay functionality |
