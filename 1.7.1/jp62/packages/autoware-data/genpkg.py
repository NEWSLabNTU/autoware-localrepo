...
def main():
    parser = argparse.ArgumentParser(
        ...
        epilog='''
Examples:
  %(prog)s --download 1.7.1           # Download tasks.yaml and generate debian/
  %(prog)s --version 1.7.1            # Generate debian/ using existing tasks.yaml
  %(prog)s -o /path/to/pkg            # Generate debian/ in specified directory
  %(prog)s --download-only 1.7.1      # Only download tasks.yaml, don't generate
'''
    )
    ...
    parser.add_argument(
        '-d', '--download',
        metavar='TAG',
        help='Download tasks.yaml from Autoware repo for specified tag (e.g., 1.7.1, 2025.02)'
    )
